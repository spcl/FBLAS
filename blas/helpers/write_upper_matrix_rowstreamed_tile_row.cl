/**
    ...
    Writes a triangular upper matrix of type TYPE_T in memory reading it
...
__kernel void WRITE_MATRIX(__global TYPE_T *restrict matrix,int N,unsigned int lda)
{
    ...
    for(int ti=0;ti<BlocksN;ti++)
    {
        for(int tj=ti;tj<BlocksN;tj++)
        {
            ...
            for(int i = 0; i < TILE_N; i++)
            {
                ...
                const int empty_A_blocks=TILE_N/W-outer_loop_limit; //in this case we receive only the meaningful part of a matrix
                ...
                for(int j=0;j<outer_loop_limit;j++)
                {
                    ...
                    for(int jj= 0; jj < W; jj++)
                    {
                        int j_idx=empty_A_blocks*W+tj*TILE_N+j*W+jj;//j_idx considering also the skipped blocks
