/**
    ...
    TRSM  solves one of the matrix equations
     ...
     This version solves the case: side left, A upper, non transposed.
...
#define TILE_M 32
#define W 8
#define KERNEL_NAME trsm_v2
#define __STRATIX_10__

//FBLAS_PARAMETERS_END
#include <commons.h>

__kernel void KERNEL_NAME(const int N, const int M, const TYPE_T alpha, __global const TYPE_T * restrict A, const unsigned int lda,
                      __global TYPE_T * restrict B, const unsigned int ldb )
{
                ...
                for(int jj=0;jj<computing_outer_loop_limit;jj++)
                {
                    ...
                    for(int j=0;j<W;j++)
                    {
                        if(tj*TILE_M+jj*W+j<M)
                        {
                            ...
                            B[k*ldb+tj*TILE_M+jj*W+j] = prev - aki * localB[jj*W+j];
                        }
                    }
                }
            }
