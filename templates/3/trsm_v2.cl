...
__kernel void {{ routine.user_name }}(const int N, const int M, const {{ routine.type_str }} alpha, __global const {{ routine.type_str }} * restrict A, const unsigned int lda,
                      __global {{ routine.type_str }} * restrict B, const unsigned int ldb )
{
                ...
                for(int jj=0;jj<computing_outer_loop_limit;jj++)
                {
                    ...
                    for(int j=0;j<WIDTH;j++)
                    {
                        if(tj*TILE_M+jj*WIDTH+j<M)
                        {
                            ...
                            B[k*ldb+tj*TILE_M+jj*WIDTH+j] = prev - aki * localB[jj*WIDTH+j];
                        }
                    }
                }
            }
