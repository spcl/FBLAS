...
__kernel void KERNEL_NAME(const int N, const int M, const TYPE_T alpha, __global const TYPE_T * restrict A, const unsigned int lda, __global TYPE_T * restrict B, const unsigned int ldb )
{
        ...
        for(int tj=0;tj<BlocksM;tj++)
        {
            ...
            for(int jj=0;jj<computing_outer_loop_limit;jj++)
            {
                ...
                for(int j=0;j<W;j++)
                {
                    ...
                    {
                        const TYPE_T prev=(i==0)?alpha*B[i*ldb+tj*TILE_M+jj*W+j]:B[i*ldb+tj*TILE_M+jj*W+j];
            ...
            for(int k=i+1;k<N;k++)
            {
                ...
                for(int jj=0;jj<computing_outer_loop_limit;jj++)
                {
                    ...
                    for(int j=0;j<W;j++)
                    {
                        if(tj*TILE_M+jj*W+j<M)
                        {
                            const TYPE_T prev=(i==0)?alpha*B[off_idx+j]:B[off_idx+j];
                            B[off_idx+j] = prev - aki * localB[jj*W+j];
                        }
                    }
                }
            }
