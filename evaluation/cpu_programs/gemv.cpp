// GEMV
// By default, alpha and beta have value different than zero to actually compute something
...
int main(int argc, char *argv[])
{
    ...
    if(argc<7)
    {
        cerr << "Usage: "<< argv[0]<<" -n <length of the matrix/vectors> -p <single/double> -r <num runs>"<<endl;
        exit(-1);
    }
    ...
    if(double_precision){
        ...
        generate_vector<double>((double *)x,n);
    ...
    }else
    {
        ...
        generate_vector<float>((float *)x,n);
    ...
    for(int p=MIN_THREADS;p<=MAX_THREADS;p++)
    {
        ...
        for(int i=0;i<=runs;i++)
        {
            ...
            asm volatile("": : :"memory");
            if(double_precision)
                cblas_dgemv(CblasRowMajor,CblasNoTrans, n,n, 1.5, (double *)A,n,(double *)x, 1, 2, (double *)y, 1);
            else
                cblas_sgemv(CblasRowMajor,CblasNoTrans, n,n, 1.5, (float *)A,n,(float *)x, 1, 2, (float *)y, 1);
