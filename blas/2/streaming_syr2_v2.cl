    ...
    for(int ti=0; ti<BlocksN;ti++)
    {
        ...
        for(int tj=ti;tj<BlocksN;tj++)
        {
            ...
            for(int i=0;i<TILE_N;i++)
            {
                ...
                const int empty_A_blocks=TILE_N/W-reading_A_limit; //in this case we receive only the meaningful part of a matrix
                for(int j=0;j<reading_A_limit;j++)
                {
                    ...
                    for(int jj=0;jj<W;jj++)
                    {
                        ...
                        local_A[jj]=tmp1*local_x_trans[empty_A_blocks*W+j*W+jj]+tmp2*local_y_trans[empty_A_blocks*W+j*W+jj]+a;

                        if(empty_A_blocks*W+tj*TILE_N+j*W+jj<i_idx)
                            local_A[jj]=0;
